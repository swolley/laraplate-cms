...
return new class extends Migration
{
    ...
    /**
     * Populate fields_snapshot for all existing presettable records.
     */
    private function populateExistingSnapshots(): void
    {
        $presettables = DB::table('presettables')->get();

        foreach ($presettables as $presettable) {
            $snapshot = DB::table('fieldables')
                ->join('fields', 'fieldables.field_id', '=', 'fields.id')
                ->where('fieldables.preset_id', $presettable->preset_id)
                ->whereNull('fieldables.deleted_at')
                ...
                ->select([
                    ...
                    'fields.type',
                    'fields.options',
                    'fields.is_translatable',
                    'fields.is_slug',
                    'fieldables.is_required',
                    'fieldables.order_column',
                    'fieldables.default',
                ])
                ->get()
                ->map(static fn (object $row): array => [
                    ...
                    'is_translatable' => (bool) $row->is_translatable,
                    'is_slug' => (bool) $row->is_slug,
                    'pivot' => [
            ...
            DB::table('presettables')
                ...
                ->update(['fields_snapshot' => json_encode($snapshot)]);
        }
    }

    private function dropOldTriggers(): void
    {
        match (DB::getDriverName()) {
            'mysql', 'mariadb' => $this->dropMySQLOldTriggers(),
            'pgsql' => $this->dropPostgreSQLOldTriggers(),
            'sqlite' => $this->dropSQLiteOldTriggers(),
            'sqlsrv' => $this->dropSQLServerOldTriggers(),
            'oracle' => $this->dropOracleOldTriggers(),
            default => throw new RuntimeException('Unsupported database driver: ' . DB::getDriverName()),
        };
    }

    private function dropMySQLOldTriggers(): void
    {
    ...
    private function dropPostgreSQLOldTriggers(): void
    {
    ...
    private function dropSQLiteOldTriggers(): void
    {
        DB::unprepared('DROP TRIGGER IF EXISTS trg_presets_insert');
        DB::unprepared('DROP TRIGGER IF EXISTS trg_presets_update');
        DB::unprepared('DROP TRIGGER IF EXISTS trg_presets_delete');
    ...
    private function dropSQLServerOldTriggers(): void
    {
        DB::unprepared('DROP TRIGGER IF EXISTS trg_presets_insert');
        DB::unprepared('DROP TRIGGER IF EXISTS trg_presets_update');
        DB::unprepared('DROP TRIGGER IF EXISTS trg_presets_delete');
    ...
    private function dropOracleOldTriggers(): void
    {
    ...
    /**
     * Restore the original triggers from the initial migration.
     */
    private function restoreOldTriggers(): void
    {
        match (DB::getDriverName()) {
            'mysql', 'mariadb' => $this->restoreMySQLOldTriggers(),
            'pgsql' => $this->restorePostgreSQLOldTriggers(),
            'sqlite' => $this->restoreSQLiteOldTriggers(),
            'sqlsrv' => $this->restoreSQLServerOldTriggers(),
            'oracle' => $this->restoreOracleOldTriggers(),
            default => throw new RuntimeException('Unsupported database driver: ' . DB::getDriverName()),
        };
    }

    private function restoreMySQLOldTriggers(): void
    {
    ...
    private function restorePostgreSQLOldTriggers(): void
    {
        ...
        DB::unprepared('CREATE TRIGGER trg_presets_insert AFTER INSERT ON presets FOR EACH ROW EXECUTE FUNCTION sync_presettables();');
        DB::unprepared('CREATE TRIGGER trg_presets_update AFTER UPDATE ON presets FOR EACH ROW EXECUTE FUNCTION sync_presettables();');
        DB::unprepared('CREATE TRIGGER trg_presets_delete AFTER DELETE ON presets FOR EACH ROW EXECUTE FUNCTION sync_presettables();');
    }

    private function restoreSQLiteOldTriggers(): void
    {
        $this->dropSQLiteTriggers();

        DB::unprepared('
            CREATE TRIGGER trg_presets_insert AFTER INSERT ON presets
            BEGIN INSERT INTO presettables (entity_id, preset_id, deleted_at) VALUES (NEW.entity_id, NEW.id, NEW.deleted_at); END;
        ');
        DB::unprepared('
            CREATE TRIGGER trg_presets_update AFTER UPDATE ON presets
            BEGIN UPDATE presettables SET entity_id = NEW.entity_id, deleted_at = NEW.deleted_at WHERE preset_id = NEW.id; END;
        ');
        DB::unprepared('
            CREATE TRIGGER trg_presets_delete AFTER DELETE ON presets
            BEGIN DELETE FROM presettables WHERE preset_id = OLD.id; END;
        ');
    }

    private function restoreSQLServerOldTriggers(): void
    {
        $this->dropSQLServerTriggers();

        DB::unprepared('
            CREATE TRIGGER trg_presets_insert ON presets AFTER INSERT AS
            BEGIN SET NOCOUNT ON; INSERT INTO presettables (entity_id, preset_id, deleted_at) SELECT entity_id, id, deleted_at FROM inserted; END;
        ');
        DB::unprepared('
            CREATE TRIGGER trg_presets_update ON presets AFTER UPDATE AS
            BEGIN SET NOCOUNT ON; UPDATE presettables SET entity_id = i.entity_id, deleted_at = i.deleted_at FROM presettables ep INNER JOIN inserted i ON ep.preset_id = i.id; END;
        ');
        DB::unprepared('
            CREATE TRIGGER trg_presets_delete ON presets AFTER DELETE AS
            BEGIN SET NOCOUNT ON; DELETE FROM presettables WHERE preset_id IN (SELECT id FROM deleted); END;
        ');
    }

    private function restoreOracleOldTriggers(): void
    {
        $this->dropOracleTriggers();

        DB::unprepared('
            CREATE OR REPLACE TRIGGER trg_presets_insert AFTER INSERT ON presets FOR EACH ROW
            BEGIN INSERT INTO presettables (entity_id, preset_id, deleted_at) VALUES (:NEW.entity_id, :NEW.id, :NEW.deleted_at); END;
        ');
        DB::unprepared('
            CREATE OR REPLACE TRIGGER trg_presets_update AFTER UPDATE ON presets FOR EACH ROW
            BEGIN UPDATE presettables SET entity_id = :NEW.entity_id, deleted_at = :NEW.deleted_at WHERE preset_id = :NEW.id; END;
        ');
        DB::unprepared('
            CREATE OR REPLACE TRIGGER trg_presets_delete AFTER DELETE ON presets FOR EACH ROW
            BEGIN DELETE FROM presettables WHERE preset_id = :OLD.id; END;
        ');
    }
};
