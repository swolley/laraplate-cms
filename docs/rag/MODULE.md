...
The generic CRUD `update` intentionally discards non-fillable input (`removeNonFillableProperties`), so it never touches these many-to-many links. Reassigning them from a client is a dedicated session-authenticated web endpoint: `POST /app/cms/contents/{content}/relations` (route name `cms.contents.relations`, `ContentsController@syncRelations`), alongside the other CMS `/app/cms/*` routes the SPA calls. It accepts optional `tags`, `categories`, `locations`, `contributors` arrays of ids; each present key is `sync`ed wholesale (an empty array clears that relation) and each omitted key is left untouched. `SyncContentRelationsAction` runs the writes in one transaction after `AuthorizationService::ensurePermission(..., 'update')` on `cms_contents`, and the content is resolved without the `global_filters` (validity) scope so drafts and expired contents remain editable. The `Categorizable` pivot models the composite `(content_id, taxonomy_id)` key through the relation's foreign/related keys — like its `Locatable`/`Contributable` siblings — rather than a model-level array `primaryKey`, which is what lets `sync()` detach existing rows.
