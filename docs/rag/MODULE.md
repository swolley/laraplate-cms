...
The generic CRUD `update` writes only fillable columns, so it never touches these many-to-many links by default. Reassigning them is done through the same generic `update` (`PATCH /app/crud/update/cms/contents`, `core.crud.replace`) by sending a `relations` map — e.g. `{ "id": 5, "relations": { "categories": [1,2], "tags": [] } }`. `Content` opts in by implementing `Modules\Core\Contracts\ProvidesSyncableRelations::syncableRelations()` (returning `tags`, `categories`, `locations`, `contributors`); that whitelist is the authorization boundary — the Core `CrudService::update` `sync`s only whitelisted many-to-many relations, in the same transaction as the column update, and rejects any other relation name (`400`). Each present relation key is synced wholesale (empty array clears it) and omitted keys are left untouched. The `Categorizable` pivot models the composite `(content_id, taxonomy_id)` key through the relation's foreign/related keys — like its `Locatable`/`Contributable` siblings — rather than a model-level array `primaryKey`, which is what lets `sync()` detach existing rows.
...
`Content` composes the cross-cutting Core lifecycle traits. `HasValidity` filters by `valid_from` / `valid_to` (the `valid()` global scope is added in `booted()`). That scope is **authoring-surface aware**: it is skipped when `authoring_surface()` is true, so the session app CRUD (`/app/crud/*`, flagged by `CrudAuthoringContextMiddleware`) returns drafts, scheduled and expired contents for staff to work on, while the public API surface (and any other query) keeps returning only currently-valid records. `Core\Models\Taxonomy` applies the same authoring-aware validity so its categories stay selectable. `HasApprovals` requires explicit approval before persisting changes; `Content` overrides `requiresApprovalWhen()` so only validity-window edits demand approval. `HasLocks` and `HasOptimisticLocking` block concurrent edits and stale writes. `SortableTrait` sorts by `order_column` (renamed `scopeOrdered` to `scopePriorityOrdered` to avoid Searchable trait clashes). `Searchable` also indexes `valid_from`, `valid_to`, and `is_deleted` so the search layer reflects publication state.
